...
var task={//类对象(没必要对其拷贝)，主要有以下函数(首字母大写)和变量(首字母小写)
    ...
    keyword_Estimate:null,//所需的关键词，用于判断是否浏览完毕。为空则浏览一定时间就返回
    keyword_TaskPage:['keyword_1',[-1]],//任务页面的关键词以及对应的控件深度(-1代表无视深度)。用于判断是否为任务页
    keyword_PackageName:[],//包名关键词，用于确认当前应用。(关键词可以不止一个
    ...
    GotoButton:function(widget){return widget;},//根据文本组件返还对应按钮。如果按钮不存在则返回null
    ClickWidget:function(widget){widget.click()},//点击控件(看情况重写)
    Estimate:function(keyword){return true;},//根据关键字判断是否完成浏览，完成浏览则返回true。keyword_Estimate为null时始终返回false
    GoBack:function(){},//根据关键词返回到任务所在页面。返回失败将抛出异常
    ExtraClick:function(){},//在返回任务所在页面后，进行新一轮任务浏览前，进行额外的点击操作。该函数用于对付恶党('恶'可以读第三声也可以读第四声)，一般不需要对其重写。点击操作失败直接抛出异常即可，反正Run函数会接的
    Finish:function(){},//供脚本结束前调用


    //【【【不需要重写】】】
    IsTargetPackage:function(){return true;},//判断当前页面是否为任务页【可直接调用】
    Run:function(){},//运行脚本。如果需要使用locator则需要将其重写，其余情况不建议对其进行修改【可直接调用】
    Attempt:function(widget){},//先尝试一次点击然后立马返回，并判断点击后这按钮是否发生变化(发生变化会返回true)。因为支付宝-饿了么经常有这种“只需点击但不需要浏览”的任务
    Swipe:function(duration,dragUp,needRandom){},//滑动屏幕。duration为拖拽用时(毫秒)，dragUp为拖拽方向(为真时向下拖拽)，needRandom为随机距离拖拽【可直接调用】
};
module.exports = task;
...
function Widget_FindIndexes(wid){//获取控件的路径。【供task.Attempt使用】
    ...
    for(let d=wid.depth();d-->0;){
        indexes.push(GetIndexInParent(wid));
        wid=wid.parent();
    }
    indexes.reverse();
    return indexes;
...
task.IsTargetPackage=function(){
    let packageName=depth(0).findOnce().packageName();
    let key_word=this.keyword_TaskPage[0];
    let key_depth=this.keyword_TaskPage[1];
    for(let pst in this.keyword_PackageName){
        let name=this.keyword_PackageName[pst];
        if(packageName.includes(name)){
            let list=textContains(key_word).find();
            for(let pst=list.length;pst-->0;){
                for(let p=key_depth.length;p-->0;){
                    if(key_depth[p]==-1 || list[pst].depth()==key_depth[p])
                        return true;
                }
            }
        }
...
task.Estimate=function(keyword){
    return keyword==null ? false : (textContains(keyword).findOnce()!=null || descContains(keyword).findOnce()!=null);
}

task.GoBack=function(){
...
//【已废弃】
task.Swipe=function(duration,dragUp,needRandom){//滑动屏幕。duration为拖拽用时(毫秒)，dragUp为拖拽方向(为真时向下拖拽)，needRandom为随机距离拖拽【可直接调用】
            ...
            try{
                ...
                if(targ){
                    ...
                    if((this.flag_Attempt<=0) || (sleep(1000),this.Attempt(targ)==false)){//对按钮尝试点击并立马返回。如果点击后按钮未发生变化说明是浏览型任务
                        this.ClickWidget(targ);
                        sleep(2000);
                        for(let tryCount=this.flag_Swipe; this.Estimate(this.keyword_Estimate)==false && tryCount>0;--tryCount){//当没完成浏览时
