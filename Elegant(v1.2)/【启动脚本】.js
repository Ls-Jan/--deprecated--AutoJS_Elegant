//配置文件Config.json的设置说明(实在不知道放哪，就放这里算了。顺带提一嘴，这配置文件不能删，文件名也不能改)：
//  【storageName】：临时存储名，不建议对其修改，而且也没啥必要改
//  【ui】：窗口ui的设置
//      【ui.alpha】：窗体透明度
//      【ui.pos】：窗口位置
//      【ui.winWidth】：窗口宽度
//
//      【ui.sep_col】：选项之间的分隔符的颜色
//      【ui.titleStyle】：标题颜色(前景+背景+字号)
//
//      【ui.optColor】：选项颜色(前景+背景+字号)
//      【ui.optDimRate】：选项禁用时黯淡的程度
//  【loopClick】：循环点击的设置
//      【loopClick.count】：点击次数
//      【loopClick.interval】：点击的时间间隔
//  【rigidClick】：机械点击的设置
//      【rigidClick.viewTime】：点进任务后停留的时长
//      【rigidClick.interval】：浏览完任务后返回到任务界面时等待的时长
//      【rigidClick.reClick】：部分任务完成后还很恶心地出现“领取”按钮，于是很贴心地设置该变量来完成点击“领取”操作。如果该值大于说明需要“领取”，并且点击完领取后会等待一段时间


var tool=require('Tools.js');
var config=tool.Storage.Load("Config");
...
var rigidClick={//控制机械(死板)点击
    ...
    Locate_ReClick:function(){},//设置再点击位置
    Run:function(){},//开始运行
}
...
loopClick.Alter_Count=function(){
    tool.Input.Integer_Positive(config.loopClick.count,false,(val)=>{
        ...
        tool.Storage.Save("Config",config);
    },'修改“点击次数”');
}
...
rigidClick.Run=function(){
    ...
    rigidClick.__locator.Callback=function(x,y){
        sleep(1000);
        pos_reClick=rigidClick.__pos_reclick!=null?rigidClick.__pos_reclick:[x,y];
        while(true){
            click(x,y),sleep(100);
            toast('【浏览任务】')
            ...
            if(config.rigidClick.reClick>0){
                toast('【再点击】')
                click(pos_reClick[0],pos_reClick[1]);
...
rigidClick.Locate_ReClick=function(){
    if(rigidClick.__pos_reclick==null){
        toast("请设置“再点击位置”");
        rigidClick.__locator.Callback=function(x,y){
            rigidClick.__pos_reclick=[x,y];
            ui.run(()=>{main.AlterMenu('[机械点击]',3,'再点击重定位：开')});
        }
        rigidClick.__locator.Run();
        toast("重定位设置完毕");
    }
    else{
        rigidClick.__pos_reclick=null;
        main.AlterMenu('[机械点击]',3,'再点击重定位：关');    
        toast("“再点击重定位”已关闭")
    }
}
...
main.Exit=function(){//结束脚本
    toast('退出脚本');
    threads.shutDownAll();
    tool.Storage.Put("operation",false);//主动退出脚本，设该值为false
    exit();
}
...
main.Run=function(){
    ...
    //【设置运行状态(仅主动退出脚本时该值才会设置为false)】
    tool.Storage.Put("operation",true);
    //【初始化任务列表】
    oneKeyTask.Init();
    //【设置菜单】
    let taskList=[];
    ...
    main.menu.__content={//菜单，设置好点击函数
        ...
        '[机械点击]':[
            ...
            ['再点击重定位：关',main.menu.opt_DoOther(rigidClick.Locate_ReClick),'config',true],
