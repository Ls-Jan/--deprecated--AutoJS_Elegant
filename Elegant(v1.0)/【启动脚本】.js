...
loopClick.__locator.Callback=function(x,y){
    ...
    for(let c=config.loopClick.count;c-->0;){
        sleep(config.loopClick.interval);
        click(x,y);
...
loopClick.Alter_Interval=function(){
    tool.input.Integer_Positive(config.loopClick.interval,false,(val)=>{
        if(val>0){
            if(val>=100){
                config.loopClick.interval=val;
                main.AlterMenu('[循环点击]',1,'时间间隔(ms)：'+val);
                toast('修改成功');
            }
            else{
...
main.Run=function(){
    ...
    main.menu.__content={//菜单，设置好点击函数
        ...
        '[循环点击]':[
            ...
            ['时间间隔(ms)：'+config.loopClick.interval,main.menu.opt_DoOther(loopClick.Alter_Interval),'config',true],
